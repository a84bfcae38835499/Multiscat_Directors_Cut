    ...
    methods(Static)
        function PreparePotentialFiles(potStructArray)
            ...
            for i=1:length(potStructArray)
                ...
                dLine4 = ['Nz = ',num2str(potStructArray(i).Nz)];
                finalstr = [dLine1,newline,dLine2,newline,dLine3,newline,dLine4,newline];
        ...
        function prepareConfigFile(confStruct)
            ...
            if(~isfile(confStruct.fileprefix+'.in_scatcond'))
                fileScatCond = fopen(confStruct.fileprefix+'.in_scatcond','w');
                fprintf(fileScatCond,'This scattering condition file was autogenerated. Default scattering conditions are those of the Cambridge SHeM');
                fprintf(fileScatCond,'\n');
                %fprintf(fileScatCond,'65,45,30');
                fprintf(fileScatCond,'63,45,0');
                fclose(fileScatCond);
            end
            fprintf(fileConf,'% s \n', confStruct.fileprefix+'.in_scatcond	! The scattering conditions input file');
